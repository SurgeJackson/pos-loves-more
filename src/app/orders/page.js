...
export default function OrdersPage() {
  ...
  const {
    ...
    mutate,
  } = useOrders(reportDate, uPos ? uPos : pos._id)
  const {
    data: payCashReportData,
    isLoading: loadingPayCashReport,
    mutate: mutatePayCashReport,
  } = usePayCashReport(reportDate, user?.admin ? '' : uPos ? uPos : pos._id)
  const {
    data: salesByGoodsReportData,
    isLoading: loadingSalesByGoodsReport,
    mutate: mutateSalesByGoodsReport,
  } = useSalesByGoodsReport(reportDate, uPos ? uPos : pos._id)
    ...
    <section className='mt-8 mx-auto w-full'>
      <div className='flex flex-col gap-2'>
        ...
        <select
          id='uPos'
          name='uPos'
          value={uPos ? uPos : pos._id}
          onChange={(ev) => setUPos(ev.target.value)}
        >
          {poses?.length > 0 &&
            poses.map((c) => (
      ...
      <div className='mt-2 flex flex-col'>
        {loadingOrders && <div>Загрузка заказов...</div>}
        {orders?.length > 0 &&
          [...orders].reverse().map((order) => (
            <div
              key={order._id}
              className='bg-gray-100 mb-2 p-2 rounded-lg grid grid-cols-4  items-center justify-center gap-2'
            >
              ...
              <div className='text-gray-500 text-xs text-center w-full text-wrap'>
                {order.cartProducts.map((product, key) => (
                  <p
                    key={product._id + product.category._id + key}
                    className='truncate'
                  >
              ...
              <div className='text-center w-full'>
                {(
                  order.cartProducts.reduce(function (tot, arr) {
                    return tot + arr.basePrice
                  }, 0) - order.discount
                ).toLocaleString()}
                &#8381;
                <div
              ...
              <div className='text-right flex flex-wrap justify-center gap-1 w-full'>
                ...
                {!order.checked && (
                  <button
                    type='button'
                    onClick={() => {
                      handleCheckClick(order._id, uPos)
                    }}
                    className='p-2 w-10 h-10'
                  >
